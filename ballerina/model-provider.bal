...
public isolated client class OpenAiModelProvider {
    *ai:ModelProvider;
    # Generated Chat Completions connector. Created only when `apiType` is `CHAT_COMPLETION`; `()` otherwise.
    ...
    public isolated function init(@display {label: "Service URL"} string serviceUrl,
        ...
        if apiType == CHAT_COMPLETION {
            chat:ConnectionConfig chatConfig = {
                auth: {api\-key: apiKey},
            ...
            chat:Client|error llmClient = new (chatConfig, normalizedUrl);
    ...
    private isolated function chatViaChatCompletions(ai:ChatMessage[]|ai:ChatUserMessage messages,
        ...
        chat:inline_response_200|error response =
            llmClient->/deployments/[self.deploymentId]/chat/completions.post(request,
        ...
        if response !is chat:createChatCompletionResponse {
            ai:Error err = error ai:LlmInvalidResponseError("Unexpected (streaming) response from the Chat Completions API");
            span.close(err);
            return err;
        }
